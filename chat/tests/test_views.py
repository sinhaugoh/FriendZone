...
class ChatRoomViewTest(TestCase):
    def setUp(self):
        ...
        self.url = reverse('chat_room', kwargs={'chat_target_id': self.user2.pk})
        
        # make user1 and user2 friend
        UserRelationshipFactory.create(user1=self.user1, user2=self.user2, relation_type='friends')
        
        # make some messages
        self.message1 = Message.objects.create(sender=self.user1, receiver=self.user2)
    ...
    def test_redirectIfNotAuthenticated(self):
        ...
        self.assertRedirects(response, '/login/?next=/chat/{}/'.format(self.user2.pk))
    ...
    def test_invalidFriendIdReturn400(self):
        response = self.client.get(reverse('chat_room', kwargs={'chat_target_id': self.user3.pk}))
