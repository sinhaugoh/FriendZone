...
class IndexViewTest(TestCase):
    def setUp(self):
        ...
        UserRelationshipFactory.create(user1=self.user1, user2=self.user2, relation_type='friends')
...
class UserLoginViewTest(TestCase):
    ...
    def test_redirectToIndexOnSuccess(self):
        response = self.client.post(self.url, {'email': self.user1.email, 'password': USER_PASSWORD})
        
        self.assertRedirects(response, '/')
        
    def test_formInvalidEmail(self):
        response = self.client.post(self.url, {'email': 'INVALID@EMAIL.COM', 'password': USER_PASSWORD})
        
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'login_form', '__all__', 'Incorrect email or password. Please try again' )

    def test_formInvalidPassword(self):
        response = self.client.post(self.url, {'email': self.user1.email, 'password': 'INVALID_PASSSWORD123'})
...
class RegisterViewTest(TestCase):
    ...
    def test_loggedInUsesCorrectTemplate(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'social_media/user_registration.html')
    ...
    def test_formInvalidEmail(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'registration_form', 'email', 'Email already in use.')
    ...
    def test_formInvalidUsername(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'registration_form', 'username', 'Username already in use.')
...
class PasswordChangeViewTest(TestCase):
    ...
    def test_redirectIfNotAuthenticated(self):
        ...
        self.assertRedirects(response, '/login/?next=/account/password_change/')
...
class ProfileViewTest(TestCase):
    ...
    def test_invalidRequestReturn404(self):
        response = self.client.get(reverse('profile', kwargs={'username': 'INVALID_USERNAME'}))
    ...
    def test_validRequestReturnCorrectContext(self):
        ...
        self.assertEqual(response.context['profile_image_url'], self.user2.profile_image.url)
...
class ProfileUpdateViewTest(TestCase):
    ...
    def test_formInitialValueCorrect(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['profile_update_form'].initial['email'], self.user1.email)
        self.assertEqual(response.context['profile_update_form'].initial['username'], self.user1.username)
        self.assertEqual(response.context['profile_update_form'].initial['profile_image'], self.user1.profile_image)
    ...
    def test_formInvalidEmail(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'profile_update_form', 'email', 'Email already in use.')
    ...
    def test_formInvalidUsername(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'profile_update_form', 'username', 'Username already in use.')
...
class FriendListViewTest(TestCase):
    def setUp(self):
        ...
        self.user1 = AppUserFactory.create()
        self.user2 = AppUserFactory.create()
        self.url = reverse('friend_list', kwargs={'username': self.user1.username})
    ...
    def test_urlIsWorking(self):
        response = self.client.get('/user/{}/friends/'.format(self.user1.username))
    ...
    def test_redirectIfNotAuthenticated(self):
        ...
        self.assertRedirects(response, '/login/?next=/user/{}/friends/'.format(self.user1.username))
    ...
    def test_invalidUsernameReturn404(self):
        response = self.client.get(reverse('friend_list', kwargs={'username': 'INVALID_USERNAME'}))
...
class FriendRequestsListViewTest(TestCase):
    def setUp(self):
        ...
        UserRelationshipFactory.create(user1=self.user1, user2=self.user2, relation_type='pending_user2_user1')
    ...
    def test_redirectIfNotAuthenticated(self):
        ...
        self.assertRedirects(response, '/login/?next=/account/friend_requests/')

    def test_loggedInUsesCorrectTemplate(self):
        response = self.client.get(self.url)
        
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'social_media/friend_request_list.html')
